...
// Calcula los puntos que se lleva UN equipo de UN partido, según el
// sistema_puntaje del torneo (mismo criterio que src/utils/tablaPosiciones.js
// en el backend). Sólo soporta el modo "goles" (fútbol); si el partido no
// está jugado devuelve null.
function calcularPuntosEquipoPartido(propios, rival, sistemaPuntaje) {
  if (propios == null || rival == null) return null;
  const sp = sistemaPuntaje || {};
  const v = sp.victoria ?? 3;
  const e = sp.empate ?? 1;
  const d = sp.derrota ?? 0;
  if (propios > rival) return v;
  if (propios < rival) return d;
  return e;
}

// Fixture y Resultados consolidados de la pestaña "General": arma, por
...
async function cargarFixtureGeneralCategoria() {
  ...
  try {
    ...
    const sistemaPuntaje = data.sistema_puntaje || {};
    const descripciones = {};
    (data.jornadas || []).forEach((j) => { descripciones[j.jornada] = j.descripcion; });

    // Columnas: las subcategorías (categorías/años) presentes, en su orden.
    const columnasMap = {};
    partidos.forEach((p) => {
      if (p.subcategoria_id && !columnasMap[p.subcategoria_id]) {
        columnasMap[p.subcategoria_id] = { id: p.subcategoria_id, nombre: p.subcategoria_nombre, orden: p.subcategoria_orden ?? 999 };
      }
    });
    const columnas = Object.values(columnasMap).sort((a, b) => (a.orden - b.orden) || String(a.nombre).localeCompare(String(b.nombre)));
    ...
    const clavesJornada = Object.keys(jornadas).sort((a, b) => {
...
function marcaOrigenLlave(motivo) {
...
function renderLlaveBracket(partidos, avances) {
  ...
  jornadas.forEach((j) => {
    ...
    partidosRonda.forEach((p) => {
      const centrosPadres = [p.equipo_local_id, p.equipo_visitante_id]
  ...
  const columnas = jornadas.map((j) => {
    ...
    const tarjetas = partidosRonda.map((p) => {
      const jugado = p.estado === 'jugado';
      const ganadorId = jugado ? determinarGanadorCliente(p) : null;
      const origenLocal = origenPorEquipoYJornada.get(`${p.equipo_local_id}|${j}`);
      const origenVisitante = origenPorEquipoYJornada.get(`${p.equipo_visitante_id}|${j}`);
      const marcaLocal = origenLocal && origenLocal.tipo === 'avance' ? marcaOrigenLlave(origenLocal.motivo) : '';
      const marcaVisitante = origenVisitante && origenVisitante.tipo === 'avance' ? marcaOrigenLlave(origenVisitante.motivo) : '';
      ...
      return `
        <div class="panel" data-match-id="${p.id}" style="position:absolute; top:${top}px; left:0; right:0; padding:5px 8px; font-size:11.5px; border-radius:6px;">
          <div style="display:flex; align-items:center; justify-content:space-between; gap:6px;">
            <span style="display:flex; align-items:center; gap:5px; overflow:hidden; font-weight:${ganadorId === p.equipo_local_id ? '700' : '400'};">${escudoOSwatch(p.club_local_logo_url, p.club_local_color)}<span style="overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">${escapeHtml(p.club_local_nombre)}</span>${marcaLocal}</span>
          ...
          <div style="display:flex; align-items:center; justify-content:space-between; gap:6px;">
            <span style="display:flex; align-items:center; gap:5px; overflow:hidden; font-weight:${ganadorId === p.equipo_visitante_id ? '700' : '400'};">${escudoOSwatch(p.club_visitante_logo_url, p.club_visitante_color)}<span style="overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">${escapeHtml(p.club_visitante_nombre)}</span>${marcaVisitante}</span>
    ...
    const notasAvances = avancesPendientes.map((a) => {
      ...
      return `<p class="texto-ayuda" style="margin:3px 0; font-size:11px;">↳ ${escapeHtml(a.club_nombre)} — ${etiqueta} a la próxima ronda</p>`;
    }).join('');
  ...
  if (partidosUltima.length === 1 && partidosUltima[0].estado === 'jugado') {
    const ganadorId = determinarGanadorCliente(partidosUltima[0]);
    if (ganadorId) {
      const esLocal = ganadorId === partidosUltima[0].equipo_local_id;
      ...
      columnaCampeon = `
        <div class="col-llave" style="min-width:150px; flex:0 0 auto; position:relative; height:${alturaTotal}px;">
          <div style="position:absolute; top:${topCampeon}px; left:0; right:0; display:flex; flex-direction:column; align-items:center; justify-content:center; padding:14px 10px; border-radius:8px; background:linear-gradient(180deg, rgba(250,204,21,0.16), rgba(250,204,21,0.04)); border:1px solid rgba(250,204,21,0.35);">
            ...
            <div style="margin-top:8px;">${escudoOSwatch(logoCampeon, colorCampeon)}</div>
            <p style="margin:6px 0 0; font-weight:700; text-align:center; font-size:13px;">${escapeHtml(nombreCampeon)}</p>
            <p class="texto-ayuda" style="margin:2px 0 0; font-size:11px; text-transform:uppercase; letter-spacing:0.4px;">Campeón</p>
          </div>
        </div>`;
    }
  }
...
function dibujarConectoresLlave() {
  if (!ultimoDibujoLlave) return;
  const { origenPorEquipoYJornada, jornadas, porJornada } = ultimoDibujoLlave;
  const svg = document.getElementById('llaveBracketSvg');
  const cont = document.getElementById('llaveBracketScroll');
  if (!svg || !cont) return;
  ...
  jornadas.forEach((j) => {
    porJornada.get(j).forEach((p) => {
      [p.equipo_local_id, p.equipo_visitante_id].forEach((equipoId) => {
...
if (typeof window !== 'undefined') {
  window.addEventListener('resize', () => dibujarConectoresLlave());
}
...
async function cargarConceptosCobro() {
  const tbody = document.getElementById('tablaConceptosCobro');
  tbody.innerHTML = '<tr><td colspan="4">Cargando...</td></tr>';
  try {
    const data = await apiFetch(`/liga/cobros/${cobrosTorneoId}/conceptos`);
    if (!data.conceptos.length) {
      tbody.innerHTML = '<tr><td colspan="4">Sin conceptos</td></tr>';
      return;
    }
    tbody.innerHTML = data.conceptos.map((c) => `
      <tr>
        <td>${NOMBRE_CONCEPTO_COBRO[c.tipo]}</td>
        <td><input type="number" step="0.01" min="0.01" value="${c.monto != null ? c.monto : ''}" id="montoConcepto_${c.tipo}" style="width:120px;"></td>
        <td><input type="checkbox" id="activoConcepto_${c.tipo}" ${c.activo ? 'checked' : ''}></td>
        <td><button type="button" class="btn btn-secundario btn-pequeno" onclick="guardarConceptoCobro('${c.tipo}')">Guardar</button></td>
      </tr>
    `).join('');
  } catch (err) {
    tbody.innerHTML = `<tr><td colspan="4" class="mensaje-error">Error: ${escapeHtml(err.message)}</td></tr>`;
  }
}

async function guardarConceptoCobro(tipo) {
  const monto = document.getElementById(`montoConcepto_${tipo}`).value;
  const activo = document.getElementById(`activoConcepto_${tipo}`).checked;
  if (!monto || Number(monto) <= 0) {
    alert('Ingresá un monto válido');
    return;
  }
  try {
    const data = await apiFetch(`/liga/cobros/${cobrosTorneoId}/conceptos/${tipo}`, {
      method: 'PUT',
      body: JSON.stringify({ monto: Number(monto), activo })
    });
    let mensaje = 'Concepto guardado.';
    if (data.deudas_generadas > 0) {
...
async function generarCargoMensual() {
  const periodoInput = document.getElementById('cobrosPeriodoMensual').value; // 'YYYY-MM'
  const errorEl = document.getElementById('cobrosMensualError');
  const exitoEl = document.getElementById('cobrosMensualExito');
  errorEl.classList.add('oculto');
  exitoEl.classList.add('oculto');
  if (!periodoInput) {
    errorEl.textContent = 'Elegí el período a generar';
    errorEl.classList.remove('oculto');
    return;
  }
  try {
    const data = await apiFetch(`/liga/cobros/${cobrosTorneoId}/generar-mensual`, {
      method: 'POST',
